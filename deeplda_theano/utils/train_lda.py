...
class LDA(object):
    ...
    def fit_from_file(self, dump_file='./model_params/lda_obj_vals.pkl'):
        ...
        with open(dump_file, 'rb') as fp:
             lda_obj_vals = pickle.load(fp)
        self.r = lda_obj_vals['r']
        self.n_components = lda_obj_vals['n_components']
        self.scalings_ = lda_obj_vals['scalings_']
        self.coef_ = lda_obj_vals['coef_']
        self.intercept_ = lda_obj_vals['intercept_']
        self.means = lda_obj_vals['means']
        print("\nLoading lda values completed.")
    ...
    def fit(self, X, y, X_te=None):
        ...
        # Save results of LDA values
        lda_obj_vals = dict()
        lda_obj_vals['r'] = self.r
        lda_obj_vals['n_components'] = self.n_components
        lda_obj_vals['scalings_'] = self.scalings_
        lda_obj_vals['coef_'] = self.coef_
        lda_obj_vals['intercept_'] = self.intercept_
        lda_obj_vals['means'] = self.means

        lda_obj_file = './model_params/lda_obj_vals.pkl'
        print("Writing file lda obj values to file \n (", lda_obj_file, ")")
        with open(lda_obj_file, 'wb') as fp:
            pickle.dump(lda_obj_vals, fp)
...
def fit(l_out, l_in, data, objective, y_tensor_type,
        ...
        r=1e-3, num_epochs=2, patience=20,
